...
Public Class YAMLBlueprints_msSQL
    ...
    Private RefDB As msSQLDBConnection
    Private RefDB2 As msSQLDBConnection

    Public Sub New(ByRef DBRef As String, Optional InstanceName As String = "")
        UpdateDB = New msSQLDBConnection(DBRef, InstanceName)
        RefDB = New msSQLDBConnection(DBRef, InstanceName)
        RefDB2 = New msSQLDBConnection(DBRef, InstanceName)
    End Sub

    Public Sub ImportFile(FilePath As String, ByRef LabelRef As Label, ByRef PGRef As ProgressBar)
        Dim SQL As String = ""
        Dim Count As Long = 0
        Dim Blueprints As New Dictionary(Of Long, Blueprint)
        ...
        Try
            ...
            Blueprints = Deserializer.Deserialize(Of Dictionary(Of Long, Blueprint))(InputText)
        ...
        For Each BP In Blueprints
            ...
            With BP.Value
                ...
                Call UpdateDB.ExecuteNonQuerySQL("INSERT INTO industryBlueprints VALUES (" & .blueprintTypeID & "," & .maxProductionLimit & ")")

                ' Now upload activities
                If Not IsNothing(.activities.copying) Then
                    Call UpdateDB.ExecuteNonQuerySQL("INSERT INTO industryActivities VALUES (" & .blueprintTypeID & "," & ActivityCopying & "," & .activities.copying.time & ")")
                ...
                If Not IsNothing(.activities.invention) Then
                    Call UpdateDB.ExecuteNonQuerySQL("INSERT INTO industryActivities VALUES (" & .blueprintTypeID & "," & ActivityInvention & "," & .activities.invention.time & ")")
                ...
                If Not IsNothing(.activities.manufacturing) Then
                    Call UpdateDB.ExecuteNonQuerySQL("INSERT INTO industryActivities VALUES (" & .blueprintTypeID & "," & ActivityManufacturing & "," & .activities.manufacturing.time & ")")
                ...
                If Not IsNothing(.activities.research_material) Then
                    Call UpdateDB.ExecuteNonQuerySQL("INSERT INTO industryActivities VALUES (" & .blueprintTypeID & "," & ActivityResearchMaterial & "," & .activities.research_material.time & ")")
                ...
                If Not IsNothing(.activities.research_time) Then
                    Call UpdateDB.ExecuteNonQuerySQL("INSERT INTO industryActivities VALUES (" & .blueprintTypeID & "," & ActivityResearchTime & "," & .activities.research_time.time & ")")
        ...
        msSQLQuery = New SqlCommand(msSQL, UpdateDB.DBREf)
        ...
        While msSQLReader.Read
            ...
            msSQLQuery2 = New SqlCommand(msSQL2, RefDB.DBREf)
            msSQLReader2 = msSQLQuery2.ExecuteReader()

            If Not msSQLReader2.Read Then
                ...
                Call RefDB2.ExecuteNonQuerySQL(SQL)
            End If

            msSQLReader2.Close()
            msSQLReader2 = Nothing
            msSQLQuery2 = Nothing
        End While
        ...
        While msSQLReader.Read
            ...
            SQL = SQL & msSQLReader.GetInt64(0) & ",1,1)"
            Call RefDB.ExecuteNonQuerySQL(SQL)
        End While
    ...
    Private Sub BuildIndustryTables()
        ...
        ' industryBlueprints
        Call UpdateDB.DropTable("industryBlueprints")
        ' Build table
        ...
        ' industryActivities
        Call UpdateDB.DropTable("industryActivities")
        ' Build table
        ...
        Call UpdateDB.DropTable("industryActivityMaterials")
        ' Build table
        ...
        ' industryActivityProducts 
        Call UpdateDB.DropTable("industryActivityProducts")
        ' Build table
