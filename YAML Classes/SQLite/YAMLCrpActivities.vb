...
Public Class YAMLCrpActivities
    ...
    Public Sub ImportData(FilePath As String, FileName As String, ByRef LabelRef As Label, ByRef PGRef As ProgressBar)
        ...
        SQL = SQL & "activityID INTEGER PRIMARY KEY,"
        SQL = SQL & "activityName VARCHAR(30) NOT NULL,"
        ...
        For Each DataField In Languages
            activityID = YAMLTranslationDocument.GetSQLScalarValueFromMapping("activityID", DataField)
            SQL = "INSERT INTO " & TableName & " VALUES ("
            SQL = SQL & activityID & ","
            SQL = SQL & UpdateDB.BuildSQLInsertStringValue(YAMLTranslationDocument.GetSQLScalarValueFromMapping("activityName", DataField)) & ","
            SQL = SQL & UpdateDB.BuildSQLInsertStringValue(YAMLTranslationDocument.GetSQLScalarValueFromMapping("description", DataField)) & ")"

            Call UpdateDB.ExecuteNonQuerySQL(SQL)

            Call frmMain.UpdateProgress(LabelRef, PGRef, Count, FileName & " Record " & activityID)
        Next
